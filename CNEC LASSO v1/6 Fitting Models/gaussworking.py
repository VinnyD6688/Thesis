...
def plot_results(A1, A2, A3, A4, C, counts, energy):
    P1 = []
    P2 = []
    P3 = []
    P4 = []
    ...
    plt.scatter(energy, counts, label='unknown', color='r')
...
def rough_UQ(A1, A2, A3, A4, C, counts, energy, R2):
    X = [[0]*5 for i in range(len(counts))]
    fwhm = peak_fwhm(a_fwhm, b_fwhm)
    for i in range(len(counts)):
        en = energy[i]
    ...
    sig_A4 = V[3][3]**0.5
    sig_C = V[4][4]**0.5
    UQ = [sig_A1/A1, sig_A2/A2, sig_A3/A3, sig_A4/A4, sig_C/(C*len(counts))]
    return(UQ)

if __name__ == "__main__":
    [channel, counts] = read_data('working.csv')
    ...
    plot_results(A1, A2, A3, A4, C, counts, energy)
    UQ = rough_UQ(A1, A2, A3, A4, C, counts, energy, R2)
"""
    print('A1:',A1)
    print('A2:',A2)
    print('A3:',A3)
